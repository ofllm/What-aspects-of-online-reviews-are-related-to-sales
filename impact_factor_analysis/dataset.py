...
class SalesDataset(Dataset):
    def __init__(self, datafile: str = "./data/impact_factor_analysis.xls", isTrain: bool = True, test_size: float = 0.3,
                 isPredict=False):
        # 加载所有的数据
        pd_data = pd.read_excel(datafile)
        if isPredict == False:
            # pd_data = pd.concat([pd_data] * 10, axis=0)
        ...
        self.x = pd_data.iloc[:, 1:12].to_numpy()
        y = pd_data.iloc[:, 12].to_numpy()
...
if __name__ == '__main__':
    train_dataset = SalesDataset(datafile="./data/impact_factor_analysis.xls", isTrain=True)
    test_dataset = SalesDataset(datafile="./data/impact_factor_analysis.xls", isTrain=False)
